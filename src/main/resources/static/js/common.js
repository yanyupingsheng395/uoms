var $MB = (function () {
    ...
    // 诊断系统日期插件的初始化
    function _initDatePicker(parentId, selectVal, dateStart, dateEnd) {
        var htmlCode = "" +
            "<span class=\"input-group-addon\">日期:</span>\n" +
            "<div class=\"form-group\">\n" +
            "    <input type=\"text\" class=\"form-control\" id=\"" + dateStart + "\" style=\"width: 120px;\">\n" +
            ...
            "</div>\n" +
            "<div class=\"form-group\">\n" +
            "    <input type=\"text\" class=\"form-control\" id=\"" + dateEnd + "\" style=\"width: 120px;\">\n" +
            "    <i class=\"form-group__bar\"></i>\n" +
            "</div>";

        $("#" + parentId).html("").append(htmlCode);
        if(selectVal == 'date') {
            var date = new Date();
            $("#" + dateEnd).val(format_date(date, selectVal));
            date.setMonth(date.getMonth() - 1);
            $("#" + dateStart).val(format_date(date, selectVal));
            dateInit(dateStart, dateEnd, "yyyy/mm/dd", "month");
        }else if(selectVal == 'month'){
            var date = new Date();
            $("#" + dateEnd).val(format_date(date, selectVal));
            date.setFullYear(date.getFullYear() - 1);
            if($("#" + dateStart).val() == "" || $("#" + dateStart).val() == null) {
                $("#" + dateStart).val(format_date(date, selectVal));
            }
            dateInit(dateStart, dateEnd, "yyyy/mm", "year");
        }else if(selectVal == 'year'){
            var date = new Date();
            $("#" + dateEnd).val(format_date(date, selectVal));
            date.setFullYear(date.getFullYear() - 1);
            $("#" + dateStart).val(format_date(date, selectVal));
            dateInit(dateStart, dateEnd, "yyyy", "decade");
        }
    }
    // 初始化日期插件
    function dateInit(dateStart, dateEnd, format, type) {
        $("#" + dateStart).attr("data-start-view", type);
        $("#" + dateStart).attr("data-min-view", type);
        $("#" + dateEnd).attr("data-start-view", type);
        $("#" + dateEnd).attr("data-min-view", type);

        var start = $('#' + dateStart).fdatepicker({
            format: format
    ...
    function format_date(date, type){
        ...
        if(type == "date") {
            var formatStr = curr_year + "/" + curr_month +"/"+ curr_date;
        }else if(type == "month") {
            var formatStr = curr_year + "/" + curr_month;
        }else if(type == "year") {
            var formatStr = curr_year;
        }
        return formatStr;
    }
    ...
    return {
        ...
        initDatePicker: function (parentId, selectId, dateStart, dateEnd) {
            _initDatePicker(parentId, selectId, dateStart, dateEnd);
        }
    }
})($);