...
function getUserGroupTable(planId,planType) {
    ...
    $("#userGroupTable").bootstrapTable('destroy').bootstrapTable({
        url: '/qywxActivityPlan/getUserGroupList',
        columns: [
            {
                field: 'groupName',
                title: '店铺活动机制'
...
function getSmsStatisTable(planId) {
    $("#smsStatisTable").bootstrapTable('destroy').bootstrapTable({
        url: '/qywxActivityPlan/getPlanSmsStatis',
        cache: false,
        pagination: true,
        singleSelect: true,
...
$("#next2Btn").on('click',function () {
    ...
    //加载一些推送相关的配置
    $.get("/qywxActivityPlan/getDefaultPushInfo", {}, function (r) {
        let code = "";
...
//禁用推送按钮
    ...
    $.post("/qywxActivityPlan/startPush", {planId: planId,pushMethod:pushMethod,pushPeriod:pushPeriod}, function (r) {
        if(r.code === 200) {
            //关闭loading
            $MB.loadingDesc('hide');
            //提示
