...
function getUserStatics(followUserId) {
    if(followUserId !== '') {
        $.get("/qywxActivity/getUserStatics", {headId: headId, followUserId:followUserId}, function (r) {
            var data = r.data;
...
function getOverAllInfo() {
    $.get("/qywxActivity/getOverAllInfo", {headId: headId}, function (r) {
        let data = r.data;
        //组织页面头数据
        $("#taskInfo").html('').append('<i class="mdi mdi-alert-circle-outline"></i>成功触达：'+data.successNum+'人，效果累计天数：'+data.effectDays);
        //填充其它数据
...
function getAllFollowUserList() {
    $.get("/qywxActivity/getFollowUserList", {headId: headId}, function (r) {
        let followUserId="";
...
{
    var settings = {
        url: '/qywxActivity/getConvertDetailData',
        pagination: true,
        singleSelect: true,
...
function makePushChart() {
    $.get("/qywxActivity/getPushEffectChange", {headId: headId}, function (r) {
        let data = r.data;
