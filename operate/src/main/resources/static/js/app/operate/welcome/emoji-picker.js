...
var _ConfigStorage = __webpack_require__(4);
...
(function ($, window, document) {
  ...
  */
  ...
  util.emojiInserted = function (emojiKey, menu) {
    _ConfigStorage2.default.get('emojis_recent', function (curEmojis) {
  ...
  EmojiArea.createIcon = function (emoji, menu) {
    ...
    var scaledWidth = _Config2.default.EmojiCategorySpritesheetDimens[category][1] * iconSize;
    var scaledHeight = _Config2.default.EmojiCategorySpritesheetDimens[category][0] * iconSize;
...
var EmojiPicker = function () {
  ...
  _createClass(EmojiPicker, [{
    ...
    value: function generateEmojiIconSets(options) {
      ...
      var totalColumns = undefined;
      j = 0;
      while (j < _Config2.default.EmojiCategories.length) {
        totalColumns = _Config2.default.EmojiCategorySpritesheetDimens[j][1];
        i = 0;
        while (i < _Config2.default.EmojiCategories[j].length) {
          dataItem = _Config2.default.Emoji[_Config2.default.EmojiCategories[j][i]];
          name = dataItem[1][0];
    ...
    value: function colonToUnicode(input) {
      ...
      if (!_Config2.default.rx_colons) {
        _Config2.default.init_unified();
      }
      return input.replace(_Config2.default.rx_colons, function (m) {
        var val = _Config2.default.mapcolon[m];
        if (val) {
          return val;
        } else {
          return '';
    ...
    value: function appendUnicodeAsImageToElement(element, input) {
      ...
      if (!_Config2.default.rx_codes) {
        _Config2.default.init_unified();
      }

      var split_on_unicode = input.split(_Config2.default.rx_codes);
      var _iteratorNormalCompletion = true;
      var _didIteratorError = false;
      var _iteratorError = undefined;

      try {
        for (var _iterator = Array.from(split_on_unicode)[Symbol.iterator](), _step; !(_iteratorNormalCompletion = (_step = _iterator.next()).done); _iteratorNormalCompletion = true) {
          var text = _step.value;

          val = '';
          if (_Config2.default.rx_codes.test(text)) {
            val = _Config2.default.reversemap[text];
            if (val) {
              val = ":" + val + ":";
      ...
      return input.replace(_Config2.default.rx_codes, function (m) {
        val = _Config2.default.reversemap[m];
        if (val) {
          val = ":" + val + ":";
...
exports.EmojiPicker = undefined;

__webpack_require__(1);

var _emojiPicker = __webpack_require__(0);

var _emojiPicker2 = _interopRequireDefault(_emojiPicker);

function _interopRequireDefault(obj) { return obj && obj.__esModule ? obj : { default: obj }; }

exports.EmojiPicker = _emojiPicker2.default;
