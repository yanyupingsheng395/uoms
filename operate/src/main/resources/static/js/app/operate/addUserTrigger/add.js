...
function saveData(dom) {
    ...
    if (validStep1Data( 'N' )) {
        ...
        $MB.confirm( {
            ...
            $.post( "/addUserTrigger/saveData", {
                    taskName: taskName, sendType: sendType, sourceId: sourceId,
...
function updateSmsContent() {
    if (validStep2Data()) {
        ...
        $.post( "/addUserTrigger/updateSmsContentAndContactWay", {
            headId: head_id,
            smsContent: $( "textarea[name='smsContent']" ).val(),
            contactWayId: $( "#qrId").val(),
            contactWayUrl: $( "#qrCode").val(),
            isSourceName: $( "input[name='isSourceName']:checked").val(),
            isProdName: $( "input[name='isProdName']:checked").val()
        }, function (r) {
            if (r.code == 200) {
