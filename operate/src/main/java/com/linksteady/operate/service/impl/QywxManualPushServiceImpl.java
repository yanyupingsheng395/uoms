...
public class QywxManualPushServiceImpl implements QywxManualPushService {
    ...
    public void pushMessage(Long headId) throws Exception{
        ...
        //获取头表信息
        QywxManualHeader header = qywxManualHeaderMapper.getQywxManualHeader(headId);
        //获取推送详细表信息
        List<QywxManualDetail> detail=qywxManualHeaderMapper.getQywxManualDetail(headId);
        String appId = qywxMessageService.getMpAppId();
        detail.stream().forEach(x->{
            String followerUserId = x.getFollowerUserId();
            //获得当前的总记录数
    ...
    @Override
    @Transactional(rollbackFor = Exception.class)
    public QywxManualError saveManualData(String smsContent, MultipartFile file, String mpTitle, String mpUrl, String mediaId) throws IOException, LinkSteadyException {
        boolean flag=true;
        ...
        for (int i = 0; i < gbk.size(); i++) {
            ...
            if(!gbk.get(i)[0].isEmpty()){
                String followerId=gbk.get(i)[0];
                int count = qywxManualHeaderMapper.checkFollowerId(followerId);
                if(count>0&&flag){
                    followerIds.add(followerId);
                    qywxManualDetail.setFollowerUserId(followerId);
                }else {
                    flag=false;
                    errorUserIds.add(followerId);
                }
            }
          if(!gbk.get(i)[1].isEmpty()){
              String externalUserId=gbk.get(i)[1];
              int count =qywxManualHeaderMapper.checkExternalUserId(externalUserId);
              if(count>0&&flag){
                  externalUserIds.add(externalUserId);
                  qywxManualDetail.setQywxContactId(externalUserId);
              }else {
                  flag=false;
                  errorUserIds.add(externalUserId);
              }
          }
          if(flag){
        ...
        if(!flag){
            ...
            qywxManualError.setErrorDesc("文件用户信息未查到");
            qywxManualError.setErrorUserId(errorUserIds);
        ...
        qywxManualHeader.setUserNumber(gbk.size());
