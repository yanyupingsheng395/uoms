...
public class PushWxMessageServiceImpl implements PushMessageService {
    ...
    public void push(List<DailyPushInfo> list) {
         ...
         if("Y".equals(dailyProperties.getRepeatPush()))
         {
             ...
             {
                 String value=operations.get("PUSH_"+dailyPushInfo.getUserOpenid());
                 if(null!=value&&!"".equals(value))
                 {
                     log.error("用户{}存在被重复触达的风险！！",dailyPushInfo.getUserOpenid());
                     cList.add(dailyPushInfo);
                 }else
                 {
                     nList.add(dailyPushInfo);
                     //已发送用户存入redis
                     operations.set("PUSH_"+dailyPushInfo.getUserOpenid(),dailyPushInfo.getUserOpenid(),timeout);
                 }
             }
