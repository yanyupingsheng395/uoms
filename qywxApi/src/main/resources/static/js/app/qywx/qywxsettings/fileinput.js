...
(function ($) {
    ...
    var isIE = function (ver) {
        ...
        tCaption = '<div tabindex="-1" class="form-control file-caption {class}">\n' +
            '   <span class="file-caption-ellipsis" id="titlefile">&hellip;</span>\n' +
        ...
        tModal = '<div id="{id}" class="modal fade">\n' +
          ...
          /*  '        <button type="button" class="close" data-dismiss="modal" aria-hidden="true">&times;</button>\n' +*/
            '        <h3 class="modal-title">Detailed Preview <small></small></h3>\n' +
            '      </div>\n' +
            '      <div class="modal-body">\n' +
            '        <textarea class="form-control" id="filecontent" style="font-family:Monaco,Consolas,monospace; height: {height}px;" readonly>{body}</textarea>\n' +
            '      </div>\n' +
            '    </div>\n' +
            '  </div>\n' +
            '</div>',
        ...
        tText = '<div class="file-preview-frame{frameClass}" id="{previewId}" data-fileindex="{fileindex}">\n' +
            '   <div class="file-preview-text" id="filecontent1" title="{caption}" ' + STYLE_SETTING + '>\n' +
            '       {data}\n' +
            '   </div>\n' +
            '   {footer}\n' +
            '</div>',
    ...
    FileInput.prototype = {
        ...
        renderUpload: function () {
          ...
          /*  if (!self.isUploadable || self.isDisabled) {
                content = '<button type="submit" onclick="uploadfile()" title="' + self.uploadTitle + '"class="' + css + '"' + status + '>' + self.uploadIcon + self.uploadLabel + '</button>';
            } else {*/
                content = '<a href="javascript:void(0)" onclick="uploadfile()"  title="' + self.uploadTitle + '" class="' + css + '"' + status + '>' + self.uploadIcon + self.uploadLabel + '</a>';
           /* }*/
            return content;
        }
    };
    ...
    $.fn.fileinput.locales.en = {
        ...
        msgInvalidFileType: '文件类型无效 "{name}". Only "{types}" 只支持此类型文件.',
        msgInvalidFileExtension: '文件类型无效 "{name}". Only "{types}" 只支持此类型文件.',
